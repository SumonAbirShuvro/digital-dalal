    ...
    return (
        <div style={s.root}>
            ...
            <header style={s.navbar}>
                ...
                {/* Desktop nav */}
                <nav style={s.navLinks} className="home-navlinks-desktop">
                    ...
                    <LangSwitcher />
                    <button style={{ ...s.loginBtn, background: 'transparent', border: '1.5px solid #166534', color: '#166534', marginLeft: 4 }} onClick={() => navigate('/register')}>
                        {t.navRegisterBtn}
                    </button>
                ...
                {/* Mobile nav right */}
                <div className="home-mobile-nav" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <LangSwitcher />
                    <button style={s.loginBtn} onClick={() => navigate('/login')}>{t.navLoginBtn}</button>
                    <button onClick={() => setMenuOpen(o => !o)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 6, color: '#166534' }}>
                ...
                {menuOpen && (
                    <div className="home-mobile-menu" style={{ width: '100%', borderTop: '1px solid #e5e7eb', paddingTop: 10, paddingBottom: 10, display: 'flex', flexDirection: 'column', gap: 4 }}>
                        <Link to="/about"    style={{ ...s.navLink, display: 'block', padding: '10px 12px' }} onClick={() => setMenuOpen(false)}>{t.navAbout}</Link>
                        <Link to="/services" style={{ ...s.navLink, display: 'block', padding: '10px 12px' }} onClick={() => setMenuOpen(false)}>{t.navServices}</Link>
                        <Link to="/contact"  style={{ ...s.navLink, display: 'block', padding: '10px 12px' }} onClick={() => setMenuOpen(false)}>{t.navContact}</Link>
                        <button style={{ ...s.loginBtn, background: 'transparent', border: '1.5px solid #166534', color: '#166534', margin: '4px 12px', textAlign: 'left' }} onClick={() => { navigate('/register'); setMenuOpen(false); }}>
                            {t.navRegisterBtn}
                        </button>
                    </div>
                )}
            ...
            <style>{`
                ...
                .home-navlinks-desktop { display: flex; align-items: center; gap: 4; flex-wrap: wrap; }
                .home-mobile-nav { display: none !important; }
                .home-mobile-menu { display: none !important; }
                @media (max-width: 640px) {
                    .home-navlinks-desktop { display: none !important; }
...
const s = {
    ...
    /* Navbar */
    navbar: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, background: 'white', padding: '10px clamp(12px, 3vw, 28px)', borderBottom: '1px solid #e5e7eb', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' },
    ...
    /* Hero */
    hero: { background: 'linear-gradient(135deg, #d4e8d4 0%, #e8ede4 60%, #f0ece0 100%)', padding: 'clamp(20px, 4vw, 40px) clamp(16px, 5vw, 60px) 32px' },
    ...
    /* Main */
    main: { flex: 1, padding: 'clamp(16px, 4vw, 28px) clamp(16px, 5vw, 60px) 40px' },
